<template>
  <div class="app-container user-container">
    <el-card class="box-card box-card-content">
      ...
      <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.page_limit" @pagination="getSystemLogList" />
    </el-card>
  </div>
</template>

<script>
import { getPayLog } from '@/api/log'
...
export default {
  ...
  methods: {
    ...
    getSystemLogList() {
      ...
      getPayLog(this.listQuery).then(response => {
