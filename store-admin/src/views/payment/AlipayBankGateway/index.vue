  ...
  <div class="app-container">
    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <router-link :to="{name:'CreateBankGateway'}">
      ...
      >
        ...
        <el-table-column prop="merch_id" label="商户编号" align="center" width="160px">
          <template slot-scope="scope">
            <template v-if="scope.row.editStatus" slit-scope="scope">
              <el-input v-model="scope.row.merch_id" />
            </template>
            <span v-else>{{ scope.row.merch_id }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column show-overflow-tooltip prop="product" label="产品编号" align="center">
          <template slot-scope="scope">
            <template v-if="scope.row.editStatus" slit-scope="scope">
...
<script>
import { index, update, destroy, updateStatus } from '@/api/AlipayBankGateway'
...
export default {
  ...
  methods: {
    ...
    confirmEdit(row) {
      // merch_id,key,product 不允许为空
      if (!row.merch_id || !row.key || !row.product) {
        this.$message({
          type: 'error',
          message: '不允许为空！'
        })
